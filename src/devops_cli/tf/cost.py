...
def parse_infracost_diff(
    ...
    past_monthly = float(data["pastTotalMonthlyCost"]) if "pastTotalMonthlyCost" in data else None
    diff_monthly = float(data["diffTotalMonthlyCost"]) if "diffTotalMonthlyCost" in data else None
...
def run_infracost_breakdown(
    ...
    if offline_mock or not check_binary("infracost"):
        mock_resources = [
            TFCostResource(
    ...
    proc = run_subprocess(
        ["infracost", "breakdown", "--path", str(target_dir), "--format", "json"],
        cwd=target_dir,
        capture_output=True,
        text=True,
        check=False,
...
def run_infracost_diff(
    ...
    """Execute Infracost diff against state or return deterministic offline mock."""
    target_dir = Path(directory)
    if offline_mock or not check_binary("infracost"):
        mock_resources = [
            TFCostResource(
    ...
    proc = run_subprocess(
        ["infracost", "diff", "--path", str(target_dir), "--format", "json"],
        cwd=target_dir,
        capture_output=True,
        text=True,
        check=False,
