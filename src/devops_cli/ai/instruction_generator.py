...
- **Dedicated Agent Workspace Data Isolation (`DEVOPS_CLI_DATA_DIR=./.data/agent`)**: AI agents executing CLI review sessions, background benchmarks, analysis scans, test executions, or temporary operational tasks must configure and isolate agent data artifacts under `./.data/agent` (via `DEVOPS_CLI_DATA_DIR=./.data/agent`) to separate agent work products from the default user workspace data tier.
