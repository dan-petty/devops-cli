...
2. **Mitigation & Invalidation Falsification**: Test any mitigations, guardrails, path containment checks (`is_relative_to`), file extension validation, secure OS Keyring usage, or cryptographic lockfiles (`uv.lock`, `poetry.lock`, `Cargo.lock`) disproving the defect.
