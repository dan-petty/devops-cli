...
- **Evidence-Based Only**: Only report findings for code that is **visible in the provided diff or file excerpt**. Do NOT speculate, infer, or hallucinate code that was not shown. If you cannot cite the exact line(s) from the provided content, do NOT create a finding. Do NOT report findings for non-existent files or hallucinated paths.
- **Active Code vs. Historical Evidence**: Historical audit notes, evidence packs, roadmaps, and research logs (e.g., `evidence/`, `docs/LOG.md`, `KNOWN_ISSUES.md`, `.data/`) document past investigations or resolved issues. Do NOT flag historical notes or documentation as live vulnerabilities unless active source code exhibits the flaw.
...
- **Language & Runtime Standards**: Inspect for language-appropriate syntax compatibility, target runtime features, and unhandled import or compilation errors. Before reporting a syntax bug (e.g. multi-exception clauses), **verify the actual code** in the provided excerpt uses the invalid form. Never hallucinate legacy syntax when valid modern syntax (e.g., `except (Err1, Err2):`) is present.
...
- **Secret Redaction Compliance**: Input code/diffs may contain automated pre-submission secret redaction placeholders (e.g., `<masked-*>`, `[REDACTED]`, `${{ secrets.* }}`, or environment masking placeholders). Treat these as intentional automated redactions; do NOT flag placeholder strings as hardcoded plain-text credentials.
...
## Feedback, Review & Self-Improvement Loop
- Ensure findings provide specific, verifiable technical assertions.
...
- Unvalidated or false-positive findings must be flagged during human review via `devops review verify` to enrich the continuous feedback benchmark dataset (`export-feedback`).
