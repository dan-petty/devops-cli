...
- **Security & Path Containment**: Enforce path containment (`is_relative_to` / canonical bounds) on filesystem writes and inputs to prevent path traversal (CWE-22). Validate path parameters using `validate_no_path_traversal` and reject targeting `is_forbidden_system_path`.
