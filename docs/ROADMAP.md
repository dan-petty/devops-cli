...
- [ ] **Executable Verification Criteria & Evidence-Backed Confidence (P0 - Critical)**:
  - *Context & Rationale*: Personas emit `verification_criteria` as deterministic shell assertions (`git check-ignore -v <path>`, `git ls-files`, `grep -n <symbol> <file>`), but nothing executes them. They are stored, merged, and **counted**: `confidence_score = len(verified_criteria_matched) / len(verification_criteria)`, where the numerator is the model's own claim about its own criteria. Confidence therefore measures self-agreement, not evidence, and every finding in the reference run carried `verified_by: "llm"`.
  - *Remediation*: Execute each criterion in the existing bounded-subprocess sandbox (`review_environment`), attach real captured output to the finding, and derive confidence from executed outcomes. Criteria that cannot be executed mark the finding `UNVERIFIED` and are reported separately rather than mixed into confirmed findings. Constrain generated criteria to a closed allowlist of read-only commands so execution is safe by construction.
...
- [ ] **Closed-Loop Hallucination Ledger (P1 - High)**:
...
|  | Executable Verification Criteria & Evidence-Backed Confidence | Bounded Subprocess Sandbox | High | High | v0.2.23 | 📋 Scheduled (P0) |
