...
- [x] **Gateway-Routed Reviews and Embeddings Across Every Inference Backend (`#458`) (P2 - Medium)**:
  ...
  - *Remaining*: Ollama traffic from the gateway follows one pod because the `ollama` Service balances per TCP connection and LiteLLM keeps connections open; give each Ollama node a stable address the gateway can list (for example a StatefulSet with per-pod DNS). File workers are still sized from `len(ollama_urls) * ollama_max_parallel`, which means nothing for provider `gateway`; size them from a gateway concurrency setting instead.
