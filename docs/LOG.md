...
- **Enhanced AI Scratchpad Buffer & Prompt Optimization**: Integrated `ScratchpadBuffer` in multi-persona agentic review orchestrator to retain reasoning state across multi-turn diff reviews, streamlined JSON serialization, and hardened parallel review error recovery.
...
- **Security Scanner Integrations**: Integrated Aqua Trivy static scanning (`devops scan`), Red Hat Kube-linter (`devops k8s lint`), Derailed Popeye cluster sanitizer (`devops k8s audit`), and Fairwinds Pluto API checker (`devops k8s check-deprecated`).
...
- **FastMCP Tool Alignment & 7-Gate CI**: Verified and mapped 18 FastMCP tools and expanded CI pipeline to 7 sequential gates (`test`, `coverage`, `lint`, `format`, `typecheck`, `audit`, `security`).

### [2026-08-12] Release v0.1.4 Implementation & Completion
- **Default AI Metadata Analysis**: Default `--enhanced` metadata generation (pseudocode outlines, complexity, ISO timestamps), incremental `st_mtime` caching, and submodule-aware dependency scanning.
...
- **Interactive Patch Staging & Air-Gapped Bundler**: Added `devops ai review apply-patch --interactive`, `devops ai bundle-models` air-gapped weight packager, Kubernetes RBAC audit scanner (`devops k8s rbac-audit`), and live SIEM audit streaming (`devops config audit-stream`).
...
- **Multi-Cluster Context & SIEM Logging**: Multi-cluster Kubeconfig context management (`devops k8s switch-context`), SIEM audit trail logger (`AuditLogger` to `.data/logs/audit.jsonl`), automated code patch application, and standardized subcommand dry-run models.
...
- **Feedback Exporter & Line-Level Comments**: Exported invalidated review findings into benchmark JSONL datasets (`devops ai review export-feedback`), custom team persona prompt overrides (`.devops/personas/<name>.md`), headless CI keyring auth fallback, and line-level PR inline comments.
...
- **Finding Invalidation CLI & Codebase Analysis**: Structured JSON finding viewer (`devops ai review findings|verify|stats`), codebase metadata analyzer (`devops ai analyze`), modular `devops_cli.dry_run` package, and `devops ci audit` dependency scanning.
- **Prompt Injection Defense & Config Output**: Escaped boundary tags, wrapped untrusted diffs in XML delimiters, added `devops config output` environment variable spec table, and deterministic static metadata extraction (`SegmentMeta`).
