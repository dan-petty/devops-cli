...
- **Review Comment Remediation & Resolution Mandate**: Actively evaluate code review feedback from GitHub Copilot and human reviewers (`gh api repos/:owner/:repo/pulls/:number/reviews` and review threads). Apply fixes cleanly via TDD, reply directly to each review thread with technical details, and resolve the conversation on GitHub (GraphQL `resolveReviewThread`). Never leave unaddressed comments or unresolved conversations.
